...
import { isAbsoluteURL, combineURL, buildURL, isURLSameOrigin } from '@/helpers/url'
...
describe('helpers:url', () => {
  describe('buildURL', () => {
    ...
    it('should support params', () => {
      expect(buildURL('/foo', { bar: 1 })).toBe('/foo?bar=1')
    })
    ...
    it('should ignore if the only param value is null', () => {
      expect(buildURL('/foo', { x: null })).toBe('/foo')
    })
  ...
  describe('isAbsoluteURL', () => {
    it('should return true if url is absolute', () => {
      expect(isAbsoluteURL('https://github.com')).toBe(true)
      expect(isAbsoluteURL('//github.com')).toBe(true)
    })
